...
export const defaultBundledCatalog: BundledCatalog = {
...
export async function syncBundledCatalog(
  ...
  return database.transaction((transaction) =>
    syncBundledCatalogInTransaction(transaction, input))
}

export async function syncBundledCatalogInTransaction(
  database: SqlDatabase,
  input: BundledCatalog,
): Promise<CatalogSyncResult> {
  ...
  for (const exercise of catalog.exercises) {
    const entry = await database.first<{ exercise_id: number; catalog_version: number }>(`
    ...
    const legacy = entry ? null : await database.first<{ id: number }>(`
