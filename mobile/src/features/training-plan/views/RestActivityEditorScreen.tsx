...
export function RestActivityEditorScreen({ plans, busyKeys, errors, onCreate, onUpdate }: { plans: TrainingPlan[]; busyKeys: Set<string>; errors: Record<string, string>; onCreate: (planId: number, dayId: number, input: RestActivityInput) => Promise<boolean>; onUpdate: (planId: number, dayId: number, activityId: number, input: RestActivityInput) => Promise<boolean> }) {
  ...
  const planId = plan.id; const dayId = day.id; const key = activity ? `activity:update:${activity.id}` : `day:activity:add:${dayId}`; const presetCategory = categoryOptions.find((option) => option.localeCompare(category, 'pt-BR', { sensitivity: 'base' }) === 0); const customCategory = !presetCategory
  ...
  return <ScreenScrollView>
    ...
    <FormSection title="TIPO"><OptionPickerField label="Categoria" value={presetCategory ?? category} placeholder="Selecionar categoria" onPress={() => setCategoryOpen(true)} />{customCategory && <FormField label="Categoria personalizada" value={category} onChangeText={setCategory} maxLength={50} />}</FormSection>
