...
export function TrainingPlanEditorScreen({
  ...
  const key = plan ? `plan:${plan.id}` : 'plan:new'
  ...
  async function save() {
    ...
    const success = plan ? await onUpdate(plan.id, input) : await onCreate(input)
      ...
      {!!plan && (
        <>
          ...
          <TouchableOpacity
            style={styles.secondary}
            onPress={() => void (async () => {
              if (await onDuplicate(plan.id)) navigation.goBack()
            })()}
          >
            ...
            onPress={() => Alert.alert(
              ...
              [
                { text: 'Cancelar', style: 'cancel' },
                {
                  ...
                  style: 'destructive',
                  onPress: () => void (async () => {
                    if (await onArchive(plan.id)) navigation.goBack()
                  })(),
                },
              ],
            )}
